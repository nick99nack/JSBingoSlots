...
/* @Created: 4:49 PM Saturday, February 13, 2021 */
...
function newBoard() {
	...
	for (var i = 0; i<5; i++){
		for (var j = 0; j<5; j++){
			boardArray[i][j] = generateNum(min_nums[i],max_nums[i]);
			if (j > 0){
				for (var k=j; k>-1; k--){
...
function endGame(mode){
	...
	if (mode == 1){
		setTimeout(function() {
			...
			document.getElementById("scoredisplay").innerHTML = score;
			mPlay("fc_snd");
			setTimeout(function() {mPlay("gameover_snd");
...
function finalSpins(){
	...
	if (spin == 16 && freespins == 0){
		setTimeout(function() {
			...
			setTimeout(function() { mPlay("scorereduce_snd"); 
			score -= 500;
			document.getElementById("scoredisplay").innerHTML = score;
			startNextSpin();
